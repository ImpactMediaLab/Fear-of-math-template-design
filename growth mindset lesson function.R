...
ShinyAssessment3 <- function(input, output, session,
  ...
  ## ASSESSMENT3 is an object describing the contents and structure of the assessment itself.
  ...
  output[[cancel.name]] <- renderUI({
    ...
    actionButton(paste0(cancel.name, SHOW_ASSESSMENT3$unique), 'Cancel this thing')
  })
  ...
  output[[ui.name]] <- renderUI({
    ...
    if(ASSESSMENT3$currentPage == totalPages) {
      nextButton <- actionButton(paste0(save.name, SHOW_ASSESSMENT3$unique), 'Done')
    } else {
      nextButton <- actionButton(nextButtonName, 'Next')
    }
    ## This is the bottom row of cancel button, page number, and done number
    mainPanel(width=12,
              br(),
              buttons[pos],
              br(),
